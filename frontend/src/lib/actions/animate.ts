...
export function fadeUp(node: HTMLElement, { delay = 0, duration = 0.35 } = {}) {
	animate(node, { y: ['18px', '0px'], opacity: [0, 1] }, { duration, delay, easing: 'ease-out' });
}

export function scalePress(node: HTMLElement) {
	const down = () => animate(node, { scale: [1, 0.97] }, { duration: 0.1 });
	const up = () =>
		animate(node, { scale: [0.97, 1] }, { duration: 0.15, easing: [0.34, 1.56, 0.64, 1] });
...
export function staggerList(node: HTMLElement) {
	...
	animate(
		...
		{ duration: 0.32, delay: (i) => i * 0.055, easing: 'ease-out' }
	);
}
