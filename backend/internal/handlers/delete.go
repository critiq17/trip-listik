...
func (h *DeleteHandler) Handle(bot BotSender, msg *tgbotapi.Message) error {
	...
	if userState == state.StateWaitingDeletePlace {
		if err := h.Service.DeletePlace(msg.From.ID, msg.Text); err != nil {
			return err
		}
		h.stateManager.SetUserState(msg.Chat.ID, state.StateNormal)
		if _, err := bot.Send(tgbotapi.NewMessage(msg.Chat.ID, fmt.Sprintf("Place %s delete from list", msg.Text))); err != nil {
			return err
		}
		log.Printf("user: %s delete place", msg.From.UserName)
	}
